...
## ツール選定の基準 (CadQuery vs Blender)

基本的には **CadQuery** を優先しますが、形状の性質に応じて **Blender** の使用を提案・選択してください。

- **CadQuery が適しているケース:**
  - 機械的なパーツ、正確な寸法指定が必要な部品（嵌合部など）、パラメトリックな変更が予想される場合。
- **Blender への切り替えを検討するケース:**
  - 絵文字のような有機的な形状、スカルプト（彫刻）的な微調整が必要なモデル、CadQueryのコードで表現すると過度に複雑になり保守性が下がる形状。
...
- `from ocp_vscode import show_object`を実行し、必ずスクリプトの最後で `show_object(result_solid)` (またはプレビューしたいオブジェクトの名前) を呼び出し、エディタ上でプレビューが表示されるようにしてください（詳細はREADME.mdの「3Dプレビュー」の設定を参照）。

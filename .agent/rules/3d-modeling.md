...
- あなたは[build123d](https://build123d.readthedocs.io/)を用いて3Dプリンターに適した3Dモデルを設計するプロフェッショナルです。
- このプロジェクトは、 @README.md のように環境構築されています。
  - **重要:** build123dは専用のConda環境（`cq-conda-env`）で動作します。スクリプトを実行する際は必ず `conda run -n cq-conda-env python ...` を使用してください。
...
## ツール選定の基準 (build123d vs Blender)

基本的には **build123d** を優先しますが、形状の性質に応じて **Blender** の使用を提案・選択してください。

- **build123d が適しているケース:**
  - 機械的なパーツ、正確な寸法指定が必要な部品（嵌合部など）、パラメトリックな変更が予想される場合。
- **Blender への切り替えを検討するケース:**
  - 絵文字のような有機的な形状、スカルプト（彫刻）的な微調整が必要なモデル、コードで表現すると過度に複雑になり保守性が下がる形状。
- **Blender使用時のハイブリッドアプローチ:**
  - Blender単独ではなく、「Pythonスクリプトによる基本形状の生成」＋「手動による形状微調整（スカルプト）のガイド」を組み合わせて提案してください。
  - **Geometry Nodesの活用**: 繰り返しパターンや数値を調整可能な「パラメトリックな形状（例：イチゴの種、網目、複数の葉）」が必要な場合は、AIがGeometry Nodesを構築するPythonスクリプトを提供してください。
  - BlenderのScriptingタブで実行可能なコードを提供し、ユーザーの初動をスムーズにしてください。
  - 最終成果物は、スライサーで扱いやすい`.3mf`形式で管理してください。

## build123dを用いたコード生成

- **ディレクトリ管理:** モデルごとに専用のディレクトリを作成し、関連ファイル（ソースコード、成果物のstepファイル、画像など）をその中に集約してください。
  - メインのPythonファイルはディレクトリ名と同名にしてください。
- `from build123d import *` および `from ocp_vscode import show_object` を行い、`show_object`関数でプレビューされるようにしてください。
- **出力形式:** Autodesk FusionやBambu Studioに直接インポート可能な `step` ファイルにしてください。
  ...
  - `export_step(result, output_path)` を使用して出力してください。
  - このタイミングで、コミット・PUSHも自動的にしてください。
- **コンテキストマネージャーの活用:** `with BuildPart() as part:`, `with BuildSketch() as sketch:`, `with BuildLine() as line:` などのコンテキストマネージャーを使用して、状態（ステート）を明確にし、構造的で保守性の高いコードを生成してください。
- **堅牢なエッジ・面選択:** `fillet` や `chamfer` 適用時、形状変化によって選択が失敗しないよう、`SortBy` などを活用した堅牢なセレクター（例: `edges().filter_by(Axis.Z)` や `faces().sort_by(SortBy.Z)[-1]` など）を心がけてください。

## モデリングのベストプラクティス

- **単位:** デフォルトである `mm` を使用してください。
- **座標系と配置:**
...
- 3DモデリングのPythonコードは「設計図」です。意図が伝わりやすくメンテナンスしやすいようにコメントを手厚く書いてください。
- 冒頭のコメントブロック（Docstring）に、**設計要件・推奨フィラメント・推奨スライサー設定・印刷統計・履歴への参照**を必ず記載してください。
...
推奨スライサー設定:
    - [日本語名] ([English name]): [値] (必要に応じて[理由・効果]を追記)
    - [日本語名] ([English name]): [値]

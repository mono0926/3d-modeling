...
# --- ネオジム磁石 (φ6.0mm × 3.0mm) ---
# ※6x3mm磁石1個、または手持ちの1.5mm磁石2枚重ね(1.5mm×2=3.0mm)に完全両対応
MAGNET_DIAMETER = 6.0       # 磁石直径
MAGNET_THICKNESS = 3.0      # 磁石厚み (3.00mm = 0.30mm × 10層)
MAGNET_HOLE_D = 6.4         # 磁石穴直径 (0.6mmノズル収縮マージン+接着剤逃げしろ)

# --- 磁石インロー構造（合計厚み6.0mm / 21層 = 6.30mm、クリアランス1層0.30mm） ---
# 本体側（オス）：穴深さ1.80mm（6層）。厚み3.0mm磁石で1.20mm突出（位置決めオスピン、横力で100%抜けない黄金比）
BODY_MAGNET_HOLE_DEPTH = 6 * LAYER_HEIGHT    # 1.80mm (オス突出: 3.00 - 1.80 = 1.20mm)

# 蓋側（メス）：穴深さ4.50mm（15層）。厚み3.0mm磁石で1.50mm窪み（メス受座、天井肉厚3.30mm確保）
LID_MAGNET_HOLE_DEPTH = 15 * LAYER_HEIGHT   # 4.50mm (メス窪み: 4.50 - 3.00 = 1.50mm)
...
def build_case_body() -> Part:
    ...
    天面四隅に磁石ポケット（深さ 1.80mm = 6層、厚み3.0mm磁石で1.20mm突出オス）。
    ...
    with BuildPart() as case:
        ...
        # 3. 天面四隅の磁石ポケットを削る（深さ 1.80mm = 6層、厚み3.0mm磁石で1.20mm突出オス）
        corner_locs = [
...
def build_top_frame() -> Part:
    ...
    Z構成:
      ...
      - Z=7.80mm (上面): 本体との合わせ面。四隅に磁石穴（深さ4.50mm = 15層、厚み3.0mm磁石でメス窪み1.50mm、天井肉厚3.30mm確保）
    """
    base_sk = create_base_sketch()
    ...
    with BuildPart() as frame:
        ...
        # 5. 上面（本体との合わせ面 Z=7.80mm）四隅の磁石ポケットを削る (深さ 4.50mm = 15層、厚み3.0mm磁石で1.50mm窪みメス)
        corner_locs = [
...
if __name__ == "__main__":
    ...
    print(f"・磁石インロー構造: 本体6層(深さ{BODY_MAGNET_HOLE_DEPTH:.2f}mm, オス1.20mm突出) / 蓋15層(深さ{LID_MAGNET_HOLE_DEPTH:.2f}mm, メス1.50mm窪み)")
